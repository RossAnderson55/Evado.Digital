    ...
    private String ConvertToCsv ( String [ ] Data )
    {
      ...
      for ( int i = 0; i < Data.Length; i++ )
      {
        ...
        if ( i == 0 )
        {
          ...
          csvData.Append ( Evado.Digital.Model.EvcStatics.encodeCsvFirstColumn ( cell ) );
          continue;
        }
        ...
        if ( cell == null )
        {
          csvData.Append ( Evado.Digital.Model.EvcStatics.encodeCsvEmpty );
          continue;
        }
        ...
        if ( float.TryParse ( cell, out fltValue ) == true
          || cell.ToLower ( ) == "na" )
        {
          csvData.Append ( Evado.Digital.Model.EvcStatics.encodeCsvNumeric ( cell ) );
        }
        else
        {
          csvData.Append ( Evado.Digital.Model.EvcStatics.encodeCsvText ( cell ) );
        }
    ...
    private String ExportTableSpacer ( EdRecordField FormField )
    {
      ...
      for ( int row = 0; row < FormField.Table.Rows.Count; row++ )
      {
        for ( int col = 0; col < FormField.Table.Header.Length; col++ )
        {
          if ( FormField.Table.Header [ col ].Text != String.Empty )
          {
            stTableHeader += Evado.Digital.Model.EvcStatics.encodeCsvEmpty;
          }
