...
namespace Evado.Dal.Clinical
{
  ...
  public class EvAncillaryRecords : EvDalBase
  {
    ...
    // =====================================================================================
    /// <summary>
    /// This method returns a list of Form Record Summary Object based on TrialId and SubjectId
    /// </summary>
    /// <param name="SubjectId">String: A Subject identifier</param>
    /// <param name="ProjectId">String: A Project identifier</param>
    /// <returns>List of EvFormRecordSummary: A list containing Form Record Summary object.</returns>
    /// <remarks>
    /// This method consists of following steps:
    /// 
    /// 1. Define the sql query parameters and sql query string. 
    ...
    /// 2. Execute the sql query string and store the results on datatable. 
    /// 
    /// 3. Loop through the table and extract the data row on FormRecordSummary object. 
    /// 
    ...
    public List<EdRecordSummary> getRecordSummary( string ProjectId, string SubjectId )
    {
      ...
      // 
      // Group by SubjectId if it exists
      // 
      ...
      using ( DataTable table = EvSqlMethods.RunQuery( _sqlQueryString, cmdParms ) )
      {
        ...
        for ( int count = 0; count < table.Rows.Count; count++ )
        {
          ...
          summary.OpenQueriedRecords = EvSqlMethods.getInteger( row, "Queried" );
          summary.LockedRecords = EvSqlMethods.getInteger( row, "Completed_Record" );
          summary.CancelledRecords = EvSqlMethods.getInteger( row, "Cancelled" );
      ...
      this.LogValue( "summaryList Count: " + summaryList.Count.ToString( ) );

      // 
      // Return a list containing an EvFormRecordSummary object.
      // 
      return summaryList;
