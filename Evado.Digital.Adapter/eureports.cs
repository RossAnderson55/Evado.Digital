...
namespace Evado.Digital.Adapter
{
  ...
  public class EuReports : EuClassAdapterBase
  {
      ...
      EvClassParameters Settings )
    {
      ...
      this._Bll_Reports = new Evado.Digital.Bll.EvReports ( this.ClassParameters );
    ...
    private Evado.Digital.Bll.EvReports _Bll_Reports = new Evado.Digital.Bll.EvReports ( );
    ...
    //  ------------------------------------------------------------------------------
    private Evado.UniForm.Model.AppData getReports_ListObject (
      Evado.UniForm.Model.Command PageCommand )
    {
      ...
      Evado.UniForm.Model.Page Page )
    {
      ...
      try
      {
        ...
        foreach ( EdReport report in this.Session.ReportTemplateList )
        {
          ...
          Evado.UniForm.Model.Command command = pageGroup.addCommand ( stReportTitle,
            EuAdapter.ADAPTER_ID,
            EuAdapterClasses.Reports.ToString ( ),
            Evado.UniForm.Model.ApplicationMethods.Get_Object );

          command.AddParameter ( Evado.UniForm.Model.CommandParameters.Page_Id,
             Evado.Digital.Model.EdStaticPageIds.Operational_Report_Page );

          command.AddParameter ( Evado.UniForm.Model.CommandParameters.Short_Title,
            report.ReportId );
    ...
    /// <returns>Evado.UniForm.Model.AppData object</returns>
    //  ------------------------------------------------------------------------------
    private Evado.UniForm.Model.AppData getReport_Object (
      Evado.UniForm.Model.Command PageCommand )
    {
      ...
      this._OutputReport = Evado.Digital.Model.EvcStatics.getBool ( parameterValue );
      ...
      Evado.UniForm.Model.Page Page )
    {
      ...
      for ( int i = 0; i < this.Session.Report.Queries.Length; i++ )
      {
        ...
        if ( query.hasSelectionSource ( ) == false )
        {
          switch ( query.DataType )
          {
            case EdReport.DataTypes.Date:
              {
                dValue = Evado.Digital.Model.EvcStatics.getDateTime ( query.Value );
        ...
        {
          optionList = Evado.Digital.Model.EvcStatics.getArrayAsList<EvOption> ( this.getQuerySelectionList ( query ) );
      ...
      Evado.UniForm.Model.Page Page )
    {
      ...
      Evado.Digital.Model.EvcStatics.Files.saveFile ( this.UniForm_BinaryFilePath, htmlFilename, htmlReport );
      ...
      Evado.Digital.Model.EvcStatics.Files.saveFile ( this.UniForm_BinaryFilePath, csvFilename, csvReport );
      ...
      Evado.Digital.Model.EvcStatics.Files.saveFile ( this.UniForm_BinaryFilePath, csvDataFilename, csvReport );
      ...
      Evado.Digital.Model.EvcStatics.Files.saveFile ( this.UniForm_BinaryFilePath, jsonDataFilename, jsonReport );
      ...
      Evado.UniForm.Model.Command Command )
    {
      try
      {
        ...
        Evado.UniForm.Model.AppData clientDataObject = new Evado.UniForm.Model.AppData ( );
        this.Session.Report = new EdReport ( );
        this.Session.Report.Guid = Evado.Digital.Model.EvcStatics.CONST_NEW_OBJECT_ID;
